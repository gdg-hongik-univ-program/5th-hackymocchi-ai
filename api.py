...
def _retrieve(state: dict) -> dict:
    """웹해킹 공격 유형별로 다중 쿼리를 날려 RAG 결과를 풍부하게 가져옴."""
    try:
        ...
        payload_queries = [
            ...
            "web login form SQL injection OR 1=1",
        ...
        for q in knowledge_queries:
            for doc in db_know.similarity_search(q, k=1):
                if id(doc) not in seen_ids:
                    seen_ids.add(id(doc))
                    all_docs.append(("know", doc))

        for q in payload_queries:
            for doc in db_pay.similarity_search(q, k=1):
                if id(doc) not in seen_ids:
                    seen_ids.add(id(doc))
                    all_docs.append(("pay", doc))

        # 페이로드 문서를 구조화된 형태로 포맷
        know_parts = []
        pay_parts = []
        for dtype, doc in all_docs:
            text = doc.page_content.strip()[:200]
            if dtype == "pay":
                pay_parts.append(f"- {text}")
            else:
                know_parts.append(f"- {text}")

        context_lines = []
        if pay_parts:
            context_lines.append("[Payload Examples from DB]")
            context_lines.extend(pay_parts[:4])
        if know_parts:
            context_lines.append("[Vulnerability Knowledge]")
            context_lines.extend(know_parts[:2])

        context_text = "\n".join(context_lines) if context_lines else _web_fallback_context(tech)

        return {"context": context_text, "_doc_count": len(all_docs)}
    except Exception as e:
        return {
