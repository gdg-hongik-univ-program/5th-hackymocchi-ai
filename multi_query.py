...
def _retrieve(state: dict) -> dict:
    """Multi-Query RAG: LLM을 이용해 다양한 관점의 검색어를 생성하여 검색 범위를 확장함."""
    try:
        ...
        # Knowledge와 Payloads 컬렉션 로드
        try:
            db_know = Chroma(persist_directory=DB_PATH, embedding_function=embeddings, collection_name=COLLECTION_KNOWLEDGE)
            db_pay = Chroma(persist_directory=DB_PATH, embedding_function=embeddings, collection_name=COLLECTION_PAYLOADS)
        except Exception:
            # DB 로드 실패 시
            return {"context": _web_fallback_context(state["detected_tech"]), "_doc_count": 0}

        tech = state["detected_tech"]
        target_url = state["target_url"]

        # 2. [Multi-Query] 쿼리 생성 (LLM 사용)
        # LLM에게 다양한 공격 벡터를 고려한 검색어를 만들게 시킵니다.
        llm = ChatOllama(model="llama3.1", temperature=0)
        
        query_gen_prompt = ChatPromptTemplate.from_template("""
        You are an AI Red Team assistant. 
        Generate 4 different search queries to retrieve hacking payloads and vulnerability info from a vector database.
        
        Target Tech: {tech}
        Target URL: {url}
        
        Requirements:
        1. Query 1: Specific CVEs or vulnerabilities for the tech.
        2. Query 2: Common exploit payloads (SQLi, XSS, etc.) suitable for this tech.
        3. Query 3: Default credentials or path enumeration for this tech.
        4. Query 4: Bypass techniques for this tech.
        
        Output: ONLY return the queries, one per line. No numbering.
        """)
        ...
        try:
            # LLM이 쿼리 생성
            generated_queries_str = chain.invoke({"tech": tech, "url": target_url})
            queries = [q.strip() for q in generated_queries_str.split('\n') if q.strip()]
            print(f"[*] Generated Queries: {queries}") # 디버깅용 (로그 확인)
        except Exception as e:
            # LLM 실패 시 기본 하드코딩 쿼리로 대체 (Fallback)
            queries = [
                f"{tech} vulnerability exploit",
                f"{tech} SQL injection payload",
                f"{tech} default password",
                "generic web attack payload"
            ]

        # 3. 검색 수행 및 중복 제거
        unique_docs = {} # Content를 키로 사용하여 중복 제거
        
        for query in queries:
            # Knowledge DB 검색
            for doc in db_know.similarity_search(query, k=2):
                if doc.page_content not in unique_docs:
                    unique_docs[doc.page_content] = "[Knowledge] " + doc.page_content

            # Payload DB 검색
            for doc in db_pay.similarity_search(query, k=2):
                if doc.page_content not in unique_docs:
                    unique_docs[doc.page_content] = "[Payload] " + doc.page_content

        # 4. 결과 정리 (컨텍스트 생성)
        retrieved_texts = list(unique_docs.values())
        
        if retrieved_texts:
            # 너무 길면 토큰 제한에 걸리므로 적당히 자름
            context_text = "\n\n".join(retrieved_texts[:8]) 
        else:
            context_text = _web_fallback_context(tech)

        return {"context": context_text, "_doc_count": len(retrieved_texts)}

    except Exception as e:
        # 치명적 오류 시 기본값 반환하여 파이프라인 중단 방지
        return {
            "context": _web_fallback_context(state["detected_tech"]) + f"\n(RAG error: {str(e)[:60]})",
            "_doc_count": 0,
        }
